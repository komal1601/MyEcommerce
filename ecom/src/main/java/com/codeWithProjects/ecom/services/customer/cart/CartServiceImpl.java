...
public class CartServiceImpl implements CartService{
	...
	public ResponseEntity<?> addProductToCart(AddProductInCartDto addProductInCartDto){
		Order activeOrder = orderRepository.findByUserIdAndStatus(AddProductInCarDto.getUserId(), OrderStatus.Pending);
		Optional<cartItem> optionalCartItems = cartItemsRepository.findByProductIdAndOrderIdAndUserId(addProductInCartDto.getProductId(), activeOrder.getId(), addProductInCartDto.getUserId());
		
		if(optionalCartItems.isPresent()) {
			return ResposeEntity.status(HttpStatus.CONFLICT).body(null);
		}
		else {
		...
		if(optionalProduct.isPresent() && optionalUser.isPresent()) {
			...
			activeOrder.serAmount(activeOrder.getAmount() + cart.getPrice());
			activeOrder.getCartItems()activeOrder.add(cart);
	...
	public OrderDto applyCoupon(Long userId, String code) {
		...
		double netAmount = activeOrder.getotalAmount() - discountAmount;
	...
	public OrderDto increaseProductQuantity(AddProductInCartDto addProductInCartDto) {
		...
		com.codeWithProjects.ecom.entity.Order activeOrder = orderRepository.findByUserIdAndStatus(AddProductInCarDto.getUserId(), OrderStatus.Pending);
		Optional<Product> optionalProduct = productRepository.findById(addProductInCartDto.getProductId());
...
//decreaseproductquantity
	...
	public OrderDto searchOrderByTrackingId(UUID trackingId) {
		Optional<jakarta.persistence.criteria.Order> optionalOrder = orderRepository.findByTrackingId(trackingId);
